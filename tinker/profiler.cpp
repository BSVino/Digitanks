...
void CProfiler::Render(CPerfBlock* pBlock, int& iLeft, int& iTop)
{
	...
	eastl::string16 sName = convertstring<char, char16_t>(pBlock->GetName());
	sName += sprintf(L": %d ms", (int)(pBlock->GetTime()*1000));
	glColor4ubv(clrBlock);
	glgui::CLabel::PaintText(sName, sName.length(), L"sans-serif", 10, (float)iLeft, (float)iTop);
