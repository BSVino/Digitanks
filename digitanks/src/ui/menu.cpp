...
void CArtilleryGamePanel::LevelChosenCallback()
{
	...
	if (CGameLobbyClient::IsInLobby())
		CGameLobbyClient::UpdateLobby(L"level", convertstring<char, char16_t>(CDigitanksGame::GetLevel(GAMETYPE_ARTILLERY, m_iLevelSelected)->GetName()));
}
...
void CStrategyGamePanel::LevelChosenCallback()
{
	...
	if (CGameLobbyClient::IsInLobby())
		CGameLobbyClient::UpdateLobby(L"level", convertstring<char, char16_t>(CDigitanksGame::GetLevel(GAMETYPE_STANDARD, m_iLevelSelected)->GetName()));
}
