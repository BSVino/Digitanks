	...
	: CPanel(0, 0, 570, 520)
{
	...
	for (size_t i = 0; i < CDigitanksGame::GetNumLevels(GAMETYPE_ARTILLERY); i++)
	{
		...
		m_pLevels->AddNode(convertstring<char, tchar>(pLevel->GetName()));
...
void CArtilleryGamePanel::LevelChosenCallback()
{
	...
	if (CGameLobbyClient::L_IsInLobby())
	{
		CGameLobbyClient::S_UpdateLobby(_T("level", convertstring<char, tchar>(CDigitanksGame::GetLevel(GAMETYPE_ARTILLERY, m_iLevelSelected)->GetName()));
...
void CArtilleryGamePanel::PreviewLevel(size_t iLevel)
{
	...
	m_pLevelDescription->SetText(tstring(_T("Author: ") + convertstring<char, tchar>(pLevel->GetAuthor()) + _T("\n \nDescription: " + convertstring<char, tchar>(pLevel->GetDescription()));
...
void CStrategyGamePanel::LevelChosenCallback()
{
	...
	if (CGameLobbyClient::L_IsInLobby())
	{
		CGameLobbyClient::S_UpdateLobby(_T("level", convertstring<char, tchar>(CDigitanksGame::GetLevel(GAMETYPE_STANDARD, m_iLevelSelected)->GetName()));
