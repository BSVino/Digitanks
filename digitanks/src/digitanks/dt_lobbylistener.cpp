...
bool CDigitanksLobbyListener::UpdatePlayer(size_t iID, const tstring& sKey, const tstring& sValue)
{
	if (sKey == "color" && sValue != "random")
	{
		...
		for (size_t i = 0; i < pLobby->GetNumPlayers(); i++)
		{
			...
			if (pPlayer->GetInfoValue("color") == sValue)
...
{
	if (CGameLobbyServer::GetLobby(iLobby)->GetInfoValue("level_file") == "")
		return false;

	if (CDigitanksGame::GetLevel(CGameLobbyServer::GetLobby(iLobby)->GetInfoValue("level_file")) == NULL)
		return false;
